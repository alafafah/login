...
function paginasi($reload, $page, $tpages) {
	...
	function replace_friendship($teks, $tipe, $useragent, $cookies){
		...
		if($httpcode['http_code']==200):
			...
			do{
				...
				$req = proccess(1, $useragent, 'friendships/'.$idv[1].'/'.$tipe.'/'.$parameters, $cookies, null, array('Accept-Language: id-ID, en-US', 'X-IG-Connection-Type: WIFI'));
	...
	function replace_media($teks, $tipe, $useragent, $cookies){
		...
		if($httpcode['http_code']==200):
			...
			if($tipe=='likers'):
				do{
					$req = proccess(1, $useragent, 'media/'.$resp->media_id.'/'.$tipe.'/', $cookies, null, array('Accept-Language: id-ID, en-US', 'X-IG-Connection-Type: WIFI'));
					$req = json_decode($req[1]);
				...
				do{
					$parameters = ($c>0) ? '?max_id='.$c : '';
					$reqs = proccess(1, $useragent, 'media/'.$resp->media_id.'/'.$tipe.'/'.$parameters, $cookies, null, array('Accept-Language: id-ID, en-US', 'X-IG-Connection-Type: WIFI'));
